 ...
 // LUCA: ridefinito il metodo eval in DecisionProcedureGuidanceJDI
    ...
    throws GuidanceException {
        if (dar instanceof DecisionAlternative_XYLOAD_GETX_Null && !this.jvm.isNull(refToLoad)) {
            ...
            final ReferenceSymbolic aliasOrigin = state.getObject(new ReferenceConcrete(dara.getAliasPosition())).getOrigin();
            ...
            if (this.jvm.isNull(refToLoad) || alreadySeen(refToLoad) ||
               !dare.getClassNameOfTargetObject().equals(this.jvm.typeOfObject(refToLoad))) {
                it.remove();
            } else {
