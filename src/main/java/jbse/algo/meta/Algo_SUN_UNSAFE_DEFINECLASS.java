...
/**
 * Meta-level implementation of {@link sun.misc.Unsafe#defineClass(String, byte[], int, int, ClassLoader, ProtectionDomain)}.
 * 
 * @author Pietro Braione
 */
//TODO simply copied from Algo_JAVA_CLASSLOADER_DEFINECLASS1. Check for differences.
public final class Algo_SUN_UNSAFE_DEFINECLASS extends Algo_INVOKEMETA_Nonbranching {
    ...
    RenameUnsupportedException {
        ...
        try {
            ...
            //gets the sixth (loader) parameter
            final Reference loaderReference = (Reference) this.data.operand(5);
            final int classLoader;
            if (state.isNull(loaderReference)) {
                //this probably means the primordial classloader
                classLoader = CLASSLOADER_BOOT;
            } else {
            	final Instance_JAVA_CLASSLOADER classLoaderInstance = (Instance_JAVA_CLASSLOADER) state.getObject(loaderReference);
