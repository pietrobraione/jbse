 ...
 */
public final class Algo_JBSE_JAVA_CONCURRENTMAP_REFINEONVALUEANDBRANCH extends Algo_INVOKEMETA<
        ...
        return (state) -> {
            ...
            try {
                ...
                if (state.isNull(this.thisReference)) {
                    ...
                    failExecution("The 'this' parameter to jbse.base.JAVA_CONCURRENTMAP.refineOnValueAndBranch method is null.");
                }
                this.map = (Instance) state.getObject(this.thisReference);
                if (this.map == null) {
                    //this should never happen
                    failExecution("The 'this' parameter to jbse.base.JAVA_CONCURRENTMAP.refineOnValueAndBranch method is symbolic and unresolved.");
                }
        ...
        return (state, alt) -> {
    		...
    		try {
    			...
    			if (alt.value()) {
    				...
    				state.pushOperand(this.valueReference);
    				snippet = state.snippetFactoryWrap()
    						.op_invokevirtual(JBSE_JAVA_CONCURRENTMAP_REFINEOUTVALUE)
    						.op_return()
    						.mk();
    			}
