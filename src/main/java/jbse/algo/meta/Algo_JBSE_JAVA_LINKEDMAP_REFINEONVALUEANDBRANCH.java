 ...
 */
public final class Algo_JBSE_JAVA_LINKEDMAP_REFINEONVALUEANDBRANCH extends Algo_INVOKEMETA<
        ...
        return (state) -> {
            ...
            try {
                ...
                if (state.isNull(this.thisReference)) {
                    ...
                    failExecution("The 'this' parameter to jbse.base.JAVA_LINKEDMAP.refineOnValueAndBranch method is null.");
                }
                this.map = (Instance) state.getObject(this.thisReference);
                if (this.map == null) {
                    //this should never happen
                    failExecution("The 'this' parameter to jbse.base.JAVA_LINKEDMAP.refineOnValueAndBranch method is symbolic and unresolved.");
                }
                this.valueReference = (Reference) this.data.operand(1);
                final Primitive javaMapContainsValue = calc.applyFunctionPrimitive(BOOLEAN, state.getHistoryPoint(), JAVA_LINKEDHASHMAP_CONTAINSVALUE.toString(), this.thisReference, this.valueReference).widen(INT).pop();
        ...
        return (state, alt) -> {
    		...
    		try {
    			...
    			if (alt.value()) {
    				...
    				state.pushOperand(this.valueReference);
    				snippet = state.snippetFactoryWrap()
    						.op_invokevirtual(JBSE_JAVA_LINKEDMAP_REFINEOUTVALUE)
    						.op_return()
    						.mk();
    			}
