...
public final class Algo_JBSE_BASE_MAKEKLASSSYMBOLIC_DO extends Algo_INVOKEMETA_Nonbranching {
    ...
    private ArrayList<Long> assumeAliasesTargets;
    ...
    private void completeSymbolic(State state, Objekt currentObjekt, HashSet<Objekt> visited) throws InvalidInputException, InvalidTypeException {
		...
		if (currentObjekt instanceof Instance || currentObjekt instanceof Klass) {
			for (Variable field : currentObjekt.fields().values()) {
				final String varName = field.getName();
				final String varType = field.getType();			
				final Value varValue = field.getValue();
				if (varValue instanceof ReferenceConcrete) {
					//makes the new value of the field
					final ReferenceSymbolic newFieldValue = (ReferenceSymbolic) state.createSymbolMemberField(varType, currentObjekt.getOrigin(), varName);

					//sets the field
					field.setValue(newFieldValue);

					//calculates the assumption
					final ReferenceConcrete ref = (ReferenceConcrete) varValue;
					if (state.isNull(ref)) {
						//it is null: record it
					...
					//calculates the assumption
					final ReferenceSymbolic ref = (ReferenceSymbolic) varValue;
					if (state.isNull(ref)) {
						//it is null
						this.assumeNull.add(newFieldValue);
					} else {
						...
						this.assumeAliasesTargets.add(state.getResolution(ref));
			...
			if (!currentArrayMemberClass.isPrimitiveOrVoid()) {
				final String currentArrayMemberType = currentArrayMemberClass.getClassName();
