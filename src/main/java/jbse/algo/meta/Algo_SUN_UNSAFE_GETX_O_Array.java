 ...
 * {@link sun.misc.Unsafe#getLongVolatile(Object, long)}, {@link sun.misc.Unsafe#getObject(Object, long)} and
...
public abstract class Algo_SUN_UNSAFE_GETX_O_Array extends Algo_INVOKEMETA<
...
StrategyUpdate<DecisionAlternative_XALOAD>> {
	...
	private final String arrayMemberCheckErrorMessage; //set by constructor
    private Reference myObjectRef;   //set by cooker
    private Simplex index;           //set by cooker
    
    public Algo_SUN_UNSAFE_GETX_O_Array(String methodName, String arrayMemberCheckErrorMessage) {
    	this.methodName = methodName;
    	this.arrayMemberCheckErrorMessage = arrayMemberCheckErrorMessage;
    }
    ...
    protected abstract boolean arrayMemberTypeCorrect(ClassFile arrayMemberType);
    ...
    protected BytecodeCooker bytecodeCooker() {
        return (state) -> {
            ...
            try {
                ...
                if (!arrayMemberTypeCorrect(arrayMemberType)) {
                    throw new UndefinedResultException("The Object o parameter to sun.misc.Unsafe." + methodName + "[Volatile] was an array whose member type is not " + this.arrayMemberCheckErrorMessage);
                }
    ...
    @Override
    protected final Class<DecisionAlternative_XALOAD> classDecisionAlternative() {
        return DecisionAlternative_XALOAD.class;
    }

    @Override
    protected final StrategyDecide<DecisionAlternative_XALOAD> decider() {
        return (state, result) -> { 
    ...
    private Value possiblyMaterialize(State state, Value val) 
    ...
    @Override
    protected final StrategyRefine_SUN_UNSAFE_GETX_Array refiner() {
        return new StrategyRefine_SUN_UNSAFE_GETX_Array() {
            ...
            throws DecisionException, ContradictionException, InvalidInputException {
            	...
            	if (accessExpression != null) {
            		state.assume(Algo_SUN_UNSAFE_GETX_O_Array.this.ctx.getCalculator().simplify(Algo_SUN_UNSAFE_GETX_O_Array.this.ctx.decisionProcedure.simplify(accessExpression)));
            	}
    ...
    protected StrategyUpdate_SUN_UNSAFE_GETX_Array updater() {
        return new StrategyUpdate_SUN_UNSAFE_GETX_Array() {
            ...
            ClasspathException, NotYetImplementedException, InvalidOperandException, InvalidInputException {
            	final Calculator calc = Algo_SUN_UNSAFE_GETX_O_Array.this.ctx.getCalculator();
                ...
                NoSuchElementException | ThreadStackEmptyException e) { //TODO propagate exceptions?
                ...
                try {
                    final boolean someTriggerFrameLoaded = 
                        Algo_SUN_UNSAFE_GETX_O_Array.this.ctx.triggerManager.loadTriggerFrames(state, calc, altResolved, Algo_SUN_UNSAFE_GETX_O_Array.this.programCounterUpdate.get());
            ...
            throws UndefinedResultException {
                throw new UndefinedResultException("The offset parameter to sun.misc.Unsafe." + Algo_SUN_UNSAFE_GETX_O_Array.this.methodName + "[Volatile] was not a correct index for the object (array) parameter");
            }
        };
    }
