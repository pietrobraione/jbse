 ...
 */
public final class Algo_JBSE_JAVA_LINKEDMAP_REFINEONKEYCOMBINATIONSANDBRANCH extends Algo_INVOKEMETA<
        ...
        return (state) -> {
            ...
            try {
                ...
                if (state.isNull(this.thisReference)) {
                    ...
                    failExecution("The 'this' parameter to jbse.base.JAVA_LINKEDMAP.refineOnKeyCombinationsAndBranch method is null.");
                }
                this.map = (Instance) state.getObject(this.thisReference);
                if (this.map == null) {
                    //this should never happen
                    failExecution("The 'this' parameter to jbse.base.JAVA_LINKEDMAP.refineOnKeyCombinationsAndBranch method is symbolic and unresolved.");
                }
                this.keysReference = (Reference) this.data.operand(1);
                if (state.isNull(this.keysReference)) {
                    //this should never happen
                    failExecution("The 'keys' parameter to jbse.base.JAVA_LINKEDMAP.refineOnKeyCombinationsAndBranch method is null.");
                }
                final Array keysArray = (Array) state.getObject(this.keysReference);
                if (keysArray == null) {
                    //this should never happen
                    failExecution("The 'keys' parameter to jbse.base.JAVA_LINKEDMAP.refineOnKeyCombinationsAndBranch method is symbolic and unresolved.");
                }
                if (keysArray.isSymbolic() || !keysArray.hasSimpleRep()) {
                    //this should never happen
                    failExecution("The 'keys' parameter to jbse.base.JAVA_LINKEDMAP.refineOnKeyCombinationsAndBranch method is a symbolic array, or has not a simple representation.");
                }
                ...
                if (this.numKeys > 10) { //TODO make this limit parametric
                	throw new MetaUnsupportedException("Excessive branching of jbse.base.JAVA_LINKEDMAP.refineOnKeyCombinationsAndBranch, 2^" + this.numKeys + " branches necessary.");
                }
