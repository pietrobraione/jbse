 ...
 */
public final class Algo_JBSE_JAVA_CONCURRENTMAP_REFINEONFRESHENTRYANDBRANCH extends Algo_INVOKEMETA<
        ...
        return (state) -> {
            ...
            try {
                ...
                if (state.isNull(this.thisReference)) {
                    ...
                    failExecution("The 'this' parameter to jbse.base.JAVA_CONCURRENTMAP.refineOnFreshEntryAndBranch method is null.");
                }
                this.map = (Instance) state.getObject(this.thisReference);
                if (this.map == null) {
                    //this should never happen
                    failExecution("The 'this' parameter to jbse.base.JAVA_CONCURRENTMAP.refineOnFreshEntryAndBranch method is symbolic and unresolved.");
                }
        ...
        return (state, alt) -> {
    		try {
    			...
    			if (alt.value()) {
    				...
    				state.pushOperand(this.thisReference);
    				snippet = state.snippetFactoryWrap()
    						.op_invokevirtual(JBSE_JAVA_CONCURRENTMAP_REFINEMAPCOMPLETE)
    						.op_return()
    						.mk();
    			}
