    ...
    throws InterruptException, ClasspathException, SymbolicValueNotAllowedException, InvalidInputException {
        ...
        try {
            ...
            if (_jzentry.isSymbolic()) {
                throw new SymbolicValueNotAllowedException("The long jzentry parameter to invocation of method java.util.zip.ZipFile.getEntryFlag cannot be a symbolic value.");
            }
            final long jzentry = ((Long) ((Simplex) _jzentry).getActualValue()).longValue();
            //TODO what if jzentry is not open?
            
            //invokes metacircularly the getEntryFlag method
            final Method method = ZipFile.class.getDeclaredMethod("getEntryFlag", long.class);
            method.setAccessible(true);
            final int retVal = (int) method.invoke(null, state.getZipFileEntryJz(jzentry));
