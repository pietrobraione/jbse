...
public class DecisionProcedureAlgorithms extends DecisionProcedureDecorator {
    ...
    throws DecisionException {
        try {
            final boolean shouldRefine;
	        final Expression accessExpressionSpecialized = (Expression) this.calc.push(accessExpression).replace(indexFormal, indexActual).pop();
    ...
    IncompatibleClassFileException, ClassFileNotAccessibleException {
        try {
            final boolean accessConcrete = (accessExpression == null);
	        final Expression accessExpressionSpecialized = (accessExpression == null ? null : (Expression) this.calc.push(accessExpression).replace(indexFormal, indexActual).pop());
            final boolean shouldRefine;
