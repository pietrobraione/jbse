...
public abstract class DecisionProcedureChainOfResponsibility implements DecisionProcedure {
    ...
    @Override
    public final boolean isSatExpands(ClassHierarchy hier, ReferenceSymbolic r, ClassFile classFile) 
    throws InvalidInputException, DecisionException {
        if (hier == null || r == null || classFile == null) {
            throw new InvalidInputException("isSatExpands invoked with a null parameter.");
        }
        final boolean localDecidesSat = isSatExpandsLocal(hier, r, classFile);
        if (localDecidesSat) {
            return delegateIsSatExpands(hier, r, classFile);
        }
        return false; //surely unsat
    }
     ...
     * the next decision procedure in the chain. 
     *  
     * @param hier see {@link #isSatExpands(ClassHierarchy, ReferenceSymbolic, ClassFile) isSatExpands}.
     * @param r see {@link #isSatExpands(ClassHierarchy, ReferenceSymbolic, ClassFile) isSatExpands}.
     * @param classFile see {@link #isSatExpands(ClassHierarchy, ReferenceSymbolic, ClassFile) isSatExpands}.
     * @return see {@link #isSatExpands(ClassHierarchy, ReferenceSymbolic, ClassFile) isSatExpands}.
     * @throws DecisionException upon failure.
     */
     ...
     * @param classFile see {@link #isSatExpands(ClassHierarchy, ReferenceSymbolic, ClassFile) isSatExpands}.
     * @return the result of invoking 
     *         {@link DecisionProcedure#isSatExpands(ClassHierarchy, ReferenceSymbolic, String) isSatExpands}{@code (hier, r, classFile)}
     *         on the next decision procedure in the chain.
     * @throws DecisionException if this decision procedure has
     *         not a successor in the chain, or if the successor
     *         fails.
     */
    private final boolean delegateIsSatExpands(ClassHierarchy hier, ReferenceSymbolic r, ClassFile classFile) 
    throws DecisionException {
        if (hasNext()) {
            try {
                return this.next.isSatExpands(hier, r, classFile);
            } catch (InvalidInputException e) {
                //this should never happen
                throw new UnexpectedInternalException(e);
            }
    ...
    @Override
    public final boolean isSatInitialized(ClassHierarchy hier, ClassFile classFile) 
    throws InvalidInputException, DecisionException {
        if (hier == null || classFile == null) {
            throw new InvalidInputException("isSatInitialized invoked with a null parameter.");
        }
        final boolean localDecidesSat = isSatInitializedLocal(hier, classFile);
        if (localDecidesSat) {
            return delegateIsSatInitialized(hier, classFile);
        }
        return false; //surely unsat
    }
    ...
    /**
     * Must be overridden by subclasses to implement 
     * {@link #isSatInitialized(ClassHierarchy, ClassFile)}. 
     * The default implementation answers {@code true} 
     * (no local decision) so the answer is generated 
     * by the next decision procedure in 
     * the chain.
     *  
     * @param hier see {@link #isSatInitialized(ClassHierarchy, ClassFile) isSatInitialized}.
     * @param classFile see {@link #isSatInitialized(ClassHierarchy, ClassFile) isSatInitialized}.
     * @return see {@link #isSatInitialized(ClassHierarchy, ClassFile) isSatInitialized}.
     * @throws DecisionException upon failure.
     */
     ...
     * @param classFile see {@link #isSatInitialized(ClassHierarchy, ClassFile) isSatInitialized}.
     * @return the result of invoking 
     *         {@link DecisionProcedure#isSatInitialized(ClassHierarchy, ClassFile) isSatInitialized}{@code (hier, classFile)}
     *         on the next decision procedure in the chain.
     * @throws DecisionException if this decision procedure has
     *         not a successor in the chain, or if the successor
     *         fails.
     */
    private final boolean delegateIsSatInitialized(ClassHierarchy hier, ClassFile classFile) 
    throws DecisionException {
        if (hasNext()) {
            try {
                return this.next.isSatInitialized(hier, classFile);
            } catch (InvalidInputException e) {
                //this should never happen
                throw new UnexpectedInternalException(e);
            }
    ...
    @Override
    public final boolean isSatNotInitialized(ClassHierarchy hier, ClassFile classFile) 
    throws InvalidInputException, DecisionException {
        if (hier == null || classFile == null) {
            throw new InvalidInputException("isSatNotInitialized invoked with a null parameter.");
        }
        final boolean localDecidesSat = isSatNotInitializedLocal(hier, classFile);
        if (localDecidesSat) {
            return delegateIsSatNotInitialized(hier, classFile);
        }
        return false; //surely unsat
    }
    ...
    /**
     * Must be overridden by subclasses to implement 
     * {@link #isSatNotInitialized(ClassHierarchy, ClassFile)}. 
     * The default implementation answers {@code true} 
     * (no local decision) so the answer is generated by 
     * the next decision procedure in the chain. 
     *  
     * @param hier see {@link #isSatNotInitialized(ClassHierarchy, ClassFile) isSatNotInitialized}.
     * @param classFile see {@link #isSatNotInitialized(ClassHierarchy, ClassFile) isSatNotInitialized}.
     * @return see {@link #isSatNotInitialized(ClassHierarchy, ClassFile) isSatNotInitialized}.
     * @throws DecisionException upon failure.
     */
     ...
     * @param classFile see {@link #isSatNotInitialized(ClassHierarchy, ClassFile) isSatNotInitialized}.
     * @return the result of invoking 
     *         {@link DecisionProcedure#isSatNotInitialized(ClassHierarchy, ClassFile) isSatNotInitialized}{@code (hier, classFile)}
     *         on the next decision procedure in the chain.
     * @throws DecisionException if this decision procedure has
     *         not a successor in the chain, or if the successor
     *         fails.
     */
    private final boolean delegateIsSatNotInitialized(ClassHierarchy hier, ClassFile classFile) 
    throws DecisionException {
        if (hasNext()) {
            try {
                return this.next.isSatNotInitialized(hier, classFile);
            } catch (InvalidInputException e) {
                //this should never happen
                throw new UnexpectedInternalException(e);
            }
