...
* [Javassist](http://www.javassist.org): JBSE uses Javassist for all the bytecode manipulation tasks. The version included in this project is 3.4.
* [JDD](http://javaddlib.sourceforge.net/jdd/): JBSE uses JDD for storing and manipulating boolean formulas. The version included in this project is 1.03.
...
Note that the Eclipse JavaCC plugin has a bug, at least up to version 1.5.27, that prevents JavaCC files to be compiled if the "Build automatically" option is active. Thus you must disable automatic build (Project > Build Automatically), then open the package explorer and navigate to src/jbse.apps.settings, right-click the file `SettingParser.jj` and select "Compile with JavaCC". Finally, you can re-enable automatic build, at least until you will need again to compile `SettingParser.jj`. If the "Compile with JavaCC" menu voice is disabled, this is because of another bug in the Eclipse JavaCC plugin that in some environments prevents it to refer the JavaCC binaries. We configured the Eclipse project to instruct the Eclipse JavaCC plugin to look for the binaries under `${eclipse_home}/plugins/sf.eclipse.javacc_1.5.27/`, but of course this works only for the Eclipse JavaCC plugin version 1.5.27. In case it does not work for you, right click on the `jbse` project in Package Explorer, select Properties, and in the Project Properties windows that pops up select on the left panel the JavaCC Options group. Under the Global Options tab you will find two text fields for the paths of the required JavaCC binaries. If you are lucky you just need to delete everything in these fields, press OK, and the Eclipse JavaCC plugin will automatically find the required binaries. If you are less lucky you will need to manually specify where the binaries are (you can find them in the JavaCC plugin directory).

### Fixing the external dependencies ###

JBSE needs an external numeric solver for pruning infeasible program paths. JBSE is currently able to communicate with [CVC3](http://www.cs.nyu.edu/acsys/cvc3/), [Sicstus](https://sicstus.sics.se), [Z3](http://z3.codeplex.com), or [CVC4](http://cvc4.cs.nyu.edu/). We strongly advise to use either Sicstus (our default choice, but you will need to buy a license), CVC4 (free), or Z3 (free for noncommercial use), because for some unclear reason the combination JBSE+CVC3 is extremely slow when compared to the others. JBSE connects to Sicstus via the Java PrologBeans library that is included with the Sicstus distribution, so you need to configure the Eclipse JBSE project build path to point to that library: Open the package explorer and right-click the jbse project, select Build Path > Configure Build Path... and under Libraries fix the link to prologbeans.jar. No build dependencies are necessary for CVC3, Z3 and CVC4 support, so if you are not using Sicstus (and you do not want to buy a license for it) you can just remove the link to prologbeans.jar in the project build path and ignore the resulting compilation errors. 

### Testing JBSE ###

Under the `tst` directory you will find a (very small) JUnit test suite. To run them you need JUnit 4.

### Deploying JBSE ###

Once the JBSE Eclipse project is compiled, you can export JBSE as a jar file and use it in your project. The jar must contain all the binaries, and possibly the source code of the files in the `jbse.meta` package and its subpackages. The latters are necessary if you want code completion, documentation and source-based debugging of the driver code. Remember that you must deploy the Javassist, JDD, and possibly the Java PrologBeans jars with JBSE. 
