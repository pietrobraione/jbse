...
JBSE needs an external numeric solver for pruning infeasible program paths. JBSE is currently able to communicate with [Sicstus](https://sicstus.sics.se), [Z3](http://z3.codeplex.com), or [CVC3](http://www.cs.nyu.edu/acsys/cvc3/). We strongly advise to use either Sicstus (our default choice, but you will need to buy a license) or Z3 (free for noncommercial use), because for some unclear reason the combination JBSE+CVC3 is extremely slow when compared to the others. JBSE connects to Sicstus via the Java PrologBeans library that is included with the Sicstus distribution, so you need to configure the Eclipse JBSE project build path to point to that library: Open the package explorer and right-click the jbse project, select Build Path > Configure Build Path... and under Libraries fix the link to prologbeans.jar. No build dependencies are necessary for Z3 and CVC3 support, so if you are not using Sicstus (and you do not want to buy a license for it) you can just remove the link to prologbeans.jar in the project build path and ignore the resulting compilation errors. 
