...
* Triggers: Triggers are user-defined instrumentation methods that JBSE executes right after making some assumption on a symbolic reference in a prescribed set, expressed as a regular expression. Triggers are meant to be used for updating ghost variables and enforcing assumptions that cannot be expressed by LICS rules. For example, if `list` has a `size` field we can enforce the precondition that `size` is equal to the number of `Node`s in the list by establishing a relationship between the initial value of `size` and the number of the nodes that JBSE progressively assumes to be present in the list during the symbolic execution. It is sufficient to inject the following instrumentation code in the `List` class:
