...
Now that we have set the essetial parameters we turn to the parameters that allow to customize the output. First, we ask JBSE to put a copy of the output in a dump file for offline inspection. At the purpose, create an `out` directory in the example project and add the following line to the `set` method:
...
* Triggers: Triggers are user-defined instrumentation methods that JBSE executes right after making some assumption on a symbolic reference in a prescribed set, expressed as a regular expression. Triggers are meant to be used for updating ghost variables and enforcing assumptions that cannot be expressed by LICS rules. For example, if the linked list has a `size` field we can establish a relationship between the initial value of this fields and the number of the nodes that JBSE assumes to be present in the list during the symbolic execution. It is sufficient to inject the following instrumentation code in the `List` class:
