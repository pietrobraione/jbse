...
Our development environment for JBSE is Eclipse.  This repository provides an Eclipse project file that can be used to build it. Currently only Eclipse project settings are available in this repository for building JBSE. You must install Eclipse plus the Git plugin (you will find it in the Eclipse Marketplace) and the Eclipse JavaCC plugin. Then import the JBSE project in Eclipse (File > Import...). Now your current workspace should have a Java project named `jbse`.

JBSE has several build dependencies, some of which are included in the `lib` subdirectory:

* [Javassist](http://www.javassist.org): JBSE uses Javassist at runtime for all the bytecode manipulation tasks. The version included in this project is 3.4.
* [JDD](http://javaddlib.sourceforge.net/jdd/): JBSE uses JDD at runtime for storing and manipulating boolean formulas. The version included in this project is 1.03.
* [JavaCC](https://java.net/projects/javacc/): Necessary for compiling the parser for the JBSE settings files. It is not needed at runtime and it is not included in this project.
* Sicstus PrologBeans (see next section).

Thus it all boils down to fixing the dependency on JavaCC, a thing that under Eclipse can be done by installing the [Eclipse JavaCC plugin](http://eclipse-javacc.sourceforge.net). Unfortunately the Eclipse JavaCC plugin has a small bug, at least up to version 1.5.27, that prevents JavaCC files to be compiled if the "Build automatically" option is active. Thus you must disable automatic build (Project > Build Automatically), then open the package explorer and navigate to `src/jbse.apps.settings`, right-click the file `SettingParser.jj` and select "Compile with JavaCC". Finally, you can re-enable automatic build and compile the Java source files generated by JavaCC. You will not usually need to compile again the parser, unless you want to hack or extend it. If the "Compile with JavaCC" menu voice is disabled, this is because of another bug in the Eclipse JavaCC plugin that in some environments prevents it to refer the JavaCC binaries. We configured the Eclipse project to instruct the Eclipse JavaCC plugin to look for the binaries under `${eclipse_home}/plugins/sf.eclipse.javacc_1.5.27/`, but of course this works only for the Eclipse JavaCC plugin version 1.5.27. In case this does not work for you, right click on the `jbse` project in Package Explorer, select Properties, and in the Project Properties windows that pops up select on the left panel the JavaCC Options group. Under the Global Options tab you will find two text fields for the paths of the required JavaCC binaries. If you are lucky you just need to delete everything in these fields, press OK, and the Eclipse JavaCC plugin will automatically find the required binaries. If you are just a bit less lucky you will need to manually specify where the binaries are (you can find them in the JavaCC plugin directory of Eclipse).

### Runtime dependencies (and one more build dependency) ###

Most of the JBSE runtime dependencies (Javassist and JDD) are already fixed by the default Eclipse project included with this distribution. But in order to have a working JBSE setup there is another runtime dependency you need to fix. JBSE needs to interact at runtime an external numeric solver for pruning infeasible program paths. At the purpose JBSE is able to communicate with [CVC3](http://www.cs.nyu.edu/acsys/cvc3/), [Sicstus](https://sicstus.sics.se), [Z3](https://github.com/Z3Prover/z3), or [CVC4](http://cvc4.cs.nyu.edu/). Since we decided to redistribute none of them with JBSE, you need to download and install at least one of them on the machine that runs JBSE. We strongly advise to use Z3, or alternatively CVC4 (both of them are free), because Sicstus and CVC3 are slower and less complete, and sooner or later we may decide to drop support to them altogether. Note that JBSE connects to Sicstus via the Java PrologBeans library that is included with the Sicstus distribution, so there is one more inevitable build and runtime dependency you need to configure: the Eclipse JBSE project build path to point to that library. Open the package explorer and right-click the jbse project, select Build Path > Configure Build Path... and under Libraries fix the link to prologbeans.jar. The exact location of this file in your filesystem depends on the path where you installed Sicstus: Please refer to the Sicstus user manual for instructions on how to locate it. Note that right now there is no way to automatically disable this dependency, so if you do not want to use nor install Sicstus the only alternative is removing the link to prologbeans.jar in the project build path so that Eclipse does not complain, and ignore the resulting compilation errors.

### Testing JBSE ###

When you are done you may try and run the (very small) JUnit test suite under the `tst` directory. At the purpose you need JUnit 4, a dependency that Eclipse fixes automatically. All tests should pass, with the possible exception of the tests in the class `jbse.dec.DecisionProcedureTest` that require Sicstus, in the case you did not install it.

### Deploying JBSE ###

Once the JBSE Eclipse project is compiled, you can export JBSE as a jar file and use it in your project. The jar must contain all the compiled binaries and possibly the source code of the files in the `jbse.meta` package and its subpackages. The latters are necessary if you want code completion, documentation and source-based debugging of the driver code. Remember that you must deploy the Javassist, JDD, and possibly the Java PrologBeans jars with JBSE. 
