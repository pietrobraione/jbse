...
* [JavaCC](https://java.net/projects/javacc/): Necessary for compiling the settings parser. Under Eclipse you can install the [Eclipse JavaCC plugin](http://eclipse-javacc.sourceforge.net).

Currently we are developing JBSE under Eclipse, and only Eclipse project settings are available right now. For this reason we advice to work under Eclipse, and install the Eclipse Git plugin (you will find it in the Eclipse Marketplace) and the Eclipse JavaCC plugin. Then import the JBSE project in Eclipse (File > Import...). To build the JavaCC files notice that there is a bug, at least up to version 1.5.27, of the Eclipse JavaCC plugin that prevents JavaCC files to be compiled if the "Build automatically" option is active. Thus you must first disable automatic build (Project > Build Automatically), then open the package explorer and navigate to src/jbse.apps.settings, right-click the file `SettingParser.jj` and select "Compile with JavaCC". Finally, you can re-enable automatic build, at least until you will need again to compile `SettingParser.jj`.

If the "Compile with JavaCC" menu voice is disabled, this is because there is another bug in the Eclipse JavaCC that prevents it to find the JavaCC binaries in some environments (it happens, e.g., with Java 8, Eclipse Luna and JavaCC plugin 1.5.27 under OSX). The Eclipse project is configured to tell the JavaCC plugin to look for the binaries under `${eclipse_home}/plugins/sf.eclipse.javacc_1.5.27/`, but of course this works only for Eclipse JavaCC plugin version 1.5.27. In case it does not work for you, right click on the jbse project in Package Explorer, select Properties, and in the Project Properties windows that pops up select on the left panel the JavaCC Options group. Under the Global Options tab you will find two text fields where you can specify the paths for the JavaCC binaries. If you are lucky you just need to delete everything in these fields, and the JavaCC plugin will automatically find the binaries. If you are less lucky you will need to manually specify where the binaries are (they are in the JavaCC plugin directory).

### Fixing the external dependencies ###

JBSE interacts with an external numeric solver for pruning infeasible program paths. Currently JBSE may interact with either [Sicstus](https://sicstus.sics.se), [Z3](http://z3.codeplex.com), or [CVC3](http://www.cs.nyu.edu/acsys/cvc3/). JBSE connects to Sicstus via the Java PrologBeans library that is included with the Sicstus distribution, so you need to configure the Eclipse JBSE project build path to point to that library (open the package explorer and right-click the jbse project, select Build Path > Configure Build Path... and under Libraries fix the link to prologbeans.jar). No build dependencies are necessary for Z3 and CVC3 support, so if you are not using Sicstus (and you do not want to buy a license for it) you can just remove the link to prologbeans.jar in the project build path and ignore the resulting compile errors. 
...
A method signature has three parts: The name in [internal classfile format](http://docs.oracle.com/javase/specs/jvms/se5.0/html/ClassFile.doc.html#14757) (`"smalldemos/ifx/IfExample"`) of the class that contains the method, a [method descriptor](http://docs.oracle.com/javase/specs/jvms/se5.0/html/ClassFile.doc.html#1169) listing the types of the parameters and of the return value (`"(I)V"`), and finally the name of the method (`"m"`).

Another essential parameter is the specification of which decision procedure JBSE must interface with in order to detect unfeasible paths. Without a decision procedure JBSE would assume all paths feasible, and thus it would erroneously report that some execution can violate the assertion. Our advice is to use either Sicstus or Z3 because the current support to CVC3 is immature. 
...
Now that we have set the essential parameters we turn to the parameters that customize the output. First, we ask JBSE to put a copy of the output in a dump file for offline inspection. At the purpose, create an `out` directory in the example project and add the following line to the `set` method:
...
An area where JBSE stands apart from all the other symbolic executors is the support to the specification of custom assumptions on symbolic inputs. Custom assumptions are indispensable to express method preconditions, data structure invariants, and in general to constrain the scope of the analysis to the relevant combinations of symbolic inputs. Let us reconsider our running example and suppose that the method `m` has a precondition stating that it cannot be invoked with a value for `x` that is less than zero. Let us also suppose that we are interested in proving the correctness of the `m` method alone, not in combination with its unspecified invokers, thus we are not interested to what happens when we invoke `m` with a value for `x` that violates the precondition. The easiest way to constrain the initial value of `x` is by injecting at the entry point of `m` a call to the `jbse.meta.Analysis.assume` method:
...
In some cases this is all one needs, but usually it is not if we want to enforce assumptions on the initial structure of the heap. The `Analysis.assume` method evaluates its argument when it is invoked. As a consequence, its indiscriminate use on symbolic references may cause an explosion in the number of paths. Let us consider, for example, a case where the method to be analyzed operates on a parameter `list` with class `List`. The class implements the singly-linked list data structure, where nodes have class `Node` and store values with type `Object`. Let's say we want to assume that the fourth value in the list is not `null`. If we follow the previous pattern and inject at the method entry point the statement `assume(list.header.next.next.next.value != null)`, the first thing JBSE will do when executing the method is to access `{ROOT}:list`, then `{ROOT}:list.header`, then `{ROOT}:list.header.next`, then `{ROOT}:list.header.next.next` and then `{ROOT}:list.header.next.next.next`. All these references are symbolic, and the heap accesses may potentially rise a `NullPointerException`. JBSE therefore must split cases and analyze what happens when any of the above references is either `null` or not `null` right at the entry of the method. In some cases this effect may cause an early combinatorial explosion of the total number of paths to be analyzed. A possible way to avoid the issue is moving the `assume` method invocation later in the code, close to the points where `{ROOT}:list.header.next.next.next.value` is possibly accessed, a procedure that is in general complex and error-prone. It would be much better if the symbolic executor were able to evaluate these kind of assumptions lazily, when the reference is first accessed, if ever, during symbolic execution. A different but related issue is that in some cases we would like to express assumptions over an arbitrarily big set of symbolic references. If, for example, we would like to assume that `list` does not store `null` values at *any* position, we should specify that *all* the symbolic references `{ROOT}:list.header.value`, `{ROOT}:list.header.next.value`, `{ROOT}:list.header.next.next.value`... are not `null`. A similar problem arises if we want to specify that the singly-linked list `list` has no loops. Expressing this kind of constraints by using `Analysis.assume` is impossible in many cases, and impractical in most of the others.
